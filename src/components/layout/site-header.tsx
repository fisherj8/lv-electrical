      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-6 px-4 py-3 sm:py-4">
        ...
        <Link
          ...
          className={cn(
            ...
            "hidden bg-brand text-white hover:bg-brand-light sm:inline-flex"
          )}
        >
          Get a Quote
        </Link>
      ...
      <nav className="flex gap-1 overflow-x-auto border-t border-brand/20 px-4 py-2 lg:hidden">
        {navLinks.map((link) => (
          <Link
            key={link.href}
            href={link.href}
            className="shrink-0 rounded-md px-3 py-1.5 text-xs font-medium text-white/80 hover:bg-brand/20 hover:text-white"
          >
            {link.label}
          </Link>
        ))}
